...
export function BarraObjetivosMovel({ objetivos, concluidos, ativo }: Props) {
    ...
    <div className="relative z-30 shrink-0 border-b-2 border-borda bg-painel">
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate text-sm font-bold text-texto">
          {atual ? atual.enunciado : concluidos >= objetivos.length ? "Fase completa!" : "Objetivos da fase"}
        </span>
      ...
      <AnimatePresence>
        {aberta && (
          ...
          >
            <ListaObjetivos objetivos={objetivos} concluidos={concluidos} ativo={ativo} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
