...
describe("ilhas", () => {
  ...
  it("a ilha seguinte abre quando a anterior tem todas as unidades prontas concluídas", () => {
    ...
    expect(estadoDaIlha(logica, { progresso: concluiu(U1, U2, U3), unidades })).toBe("bloqueada");
    expect(estadoDaIlha(logica, { progresso: concluiu(U1, U2, U3, U4), unidades })).toBe("disponivel");
  });
...
describe("zonas e unidades", () => {
  it("do zero: U1 disponível, U2/U3/U4 bloqueadas (já prontas), o resto planejado", () => {
    const fonte = { progresso: PROGRESSO_PADRAO };
  ...
  it("dentro da zona é em sequência; a zona seguinte abre com tudo pronto concluído", () => {
    ...
    expect(zonaAberta(SITES, ESTILOS, { progresso: concluiu(U1, U2, U3) })).toBe(false);
    expect(zonaAberta(SITES, ESTILOS, { progresso: concluiu(U1, U2, U3, U4) })).toBe(true);
  });
